...
# lr.coef_ is the slope of the regression line, indicating how much the salary is expected to increase for each additional year of experience.
# lr.intercept_ represents the y-intercept of the regression line, which is the expected salary when years of experience is zero. In this case, it should be close to 60,000.
print(f"Slope (Coefficient): {lr.coef_[0]:.2f}")
print(f"Intercept: {lr.intercept_:.2f}")
coefficients =  lr.coef_
intercept = lr.intercept_
X_plot = np.linspace(0, 20, 100)
y_plot = coefficients * X_plot + intercept

plt.scatter(X_plot, y_plot, color='blue', label= f'y = {coefficients[0]:.2f}x + {intercept:.2f}')
